...
#   6. ExchangeToken → GetUserInfo → 落盘 ~/.cli-proxy-api/trae-{uid}.json（带 "type":"trae"）
#   7. 自动签到 + 查积分（CPA watcher 自动加载，无需重启任何服务）
#
# 用法:
#   ./login.sh
set -euo pipefail
...
cd "$(dirname "$0")"
# 落盘目录改为 CPA 的 auth 目录（原值 "./auths"）——CPA 有 auth 文件 watcher，落盘即自动加载。
...
# ─── 落盘 ~/.cli-proxy-api/trae-{uid}.json（嵌套形 + 顶层 type:trae）───
...
import json, os
auth = {
    # 顶层 type 必填：CPA 宿主按它把凭证路由给 trae 插件；缺失时其它插件可能抢先认领。
    "type": "trae",
...
# ─── CPA 加载新账号（无需重启）─────────────────
echo ""
echo "凭证已写入: $AUTH_FILE"
echo "CPA 的 auth 文件 watcher 会自动加载，无需重启服务。"
echo "如需立即生效：launchctl kickstart -k gui/$(id -u)/com.cliproxyapi.server"
echo "验证：curl -s http://127.0.0.1:8317/v0/management/plugins/trae/accounts \\"
echo "        -H \"Authorization: Bearer $(cat ~/.cli-proxy-api/.mgmt_key 2>/dev/null || echo '<mgmt-key>')\" | head -c 400"
