...
// 从 traework2api/internal/upstream/payload.go 迁移（原样保留改写规则）。
package main
...
func PrepareBody(src []byte) []byte {
	...
	if msgs, ok := obj["messages"].([]any); ok {
		for _, mi := range msgs {
			...
			// 上游 SOLO 只接受 system/assistant/user/tool/function，拒绝
			// developer role（报 4027）。部分客户端对 reasoning 模型会把
			// system prompt 转成 developer 发送，此处归一化为 system。
			if role == "developer" {
				m["role"] = "system"
				role = "system"
			}
...
// 实测上游 function.parameters 是 string 类型（OpenAI 标准是 object）
// → 需把 parameters 对象序列化为 JSON 字符串。
